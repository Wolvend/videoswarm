...
const createEmbeddedDragIcon = () => {
  try {
    let icon = nativeImage.createFromBuffer(
      Buffer.from(dragIconBase64, "base64")
    );
    if (!icon.isEmpty()) {
    ...
    return typeof nativeImage.createEmpty === "function"
      ? nativeImage.createEmpty()
      : nativeImage.createFromBuffer(Buffer.alloc(0));
  }
};

let nativeDragIcon = createEmbeddedDragIcon();

const ensureDragIcon = () => {
  if (!nativeDragIcon || nativeDragIcon.isEmpty()) {
    nativeDragIcon = createEmbeddedDragIcon();
  }
  return nativeDragIcon;
};
